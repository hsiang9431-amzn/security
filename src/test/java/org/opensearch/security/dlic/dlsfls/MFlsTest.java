...
public class MFlsTest extends AbstractDlsFlsTest{
    ...
    public void testFlsMGetSearch() throws Exception {
        ...
        Assert.assertEquals(HttpStatus.SC_OK, (res = rh.executeGetRequest("deals,finance/_search?pretty", AbstractSecurityUnitTest.encodeBasicHeader("dept_manager_fls", "password"))).getStatusCode());
        ...
        Assert.assertEquals(HttpStatus.SC_OK, (res = rh.executePostRequest("_msearch?pretty", msearchBody, AbstractSecurityUnitTest.encodeBasicHeader("dept_manager_fls", "password"))).getStatusCode());
        Assert.assertFalse(res.getBody().contains("_opendistro_security_"));
        Assert.assertTrue(res.getBody().contains("\"failed\" : 0"));
        Assert.assertFalse(res.getBody().contains("xception"));
        Assert.assertTrue(res.getBody().contains("cust1"));
        ...
        Assert.assertEquals(HttpStatus.SC_OK, (res = rh.executePostRequest("_mget?pretty", mgetBody, AbstractSecurityUnitTest.encodeBasicHeader("dept_manager_fls", "password"))).getStatusCode());
        Assert.assertFalse(res.getBody().contains("_opendistro_security_"));
