...
public class SecurityHealthActionTest extends AbstractRestApiUnitTest {
    ...
    public void testSecurityHealthAPI() throws Exception {
        ...
        RestHelper.HttpResponse response = rh.executeGetRequest("_opendistro/_security/health");
    ...
    @Test
    public void testSecurityHealthAPIPlugins() throws Exception {
        Settings settings = Settings.builder().put(ConfigConstants.OPENDISTRO_SECURITY_UNSUPPORTED_RESTAPI_ALLOW_SECURITYCONFIG_MODIFICATION, true).build();
        setup(settings);

        rh.keystore = "restapi/kirk-keystore.jks";
        rh.sendAdminCertificate = true;
        RestHelper.HttpResponse response = rh.executeGetRequest("_plugins/_security/health");
        Assert.assertEquals(HttpStatus.SC_OK, response.getStatusCode());
